...
export async function analyticsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/analytics/overview', async (req) => {
    ...
    const [totalLeads, leadsToday, conversionsToday, followUpsDue, pipelineValue] = await Promise.all([
      fastify.prisma.lead.count({ where: { tenantId } }),
      fastify.prisma.lead.count({ where: { tenantId, createdAt: { gte: today } } }),
      fastify.prisma.lead.count({ where: { tenantId, status: { name: { in: ['Closed Won', 'Converted'] } }, updatedAt: { gte: today } } }),
  ...
  fastify.get('/analytics/lead-funnel', async (req) => {
    const tenantId = req.user.tenantId;
    const statuses = await fastify.prisma.status.findMany({ where: { tenantId: undefined, serviceBoard: { tenantId } } });
  ...
  fastify.get('/analytics/agent-performance', async (req) => {
    ...
    const performance = await Promise.all(
      agents.map(async a => ({
        ...
        overduFollowUps: await fastify.prisma.followUp.count({ where: { tenantId, assignedUserId: a.id, status: 'missed' } }),
      }))
    );
  ...
  fastify.get('/analytics/conversion-trends', async (req) => {
    ...
    const data = await fastify.prisma.$queryRaw<Array<{ date: string; count: bigint }>>`
      ...
      ORDER BY 1
    `;
    return { success: true, data: data.map(d => ({ date: d.date, count: Number(d.count) })) };
  });
  ...
  fastify.get('/analytics/overview', async (req) => {
    ...
    const [totalLeads, convertedToday, followUpsDue, pipeline, sources, trendRaw] = await Promise.all([
      fastify.prisma.lead.count({ where: { tenantId } }),
    ...
    return {
      success: true,
      data: {
        totalLeads, convertedToday, followUpsDue,
        revenuePipeline: pipeline._sum.dealValue ?? 0,
        sourceBreakdown,
        trend: trendRaw.map(d => ({ date: String(d.date).slice(5, 10), leads: Number(d.count) })),
        conversionTrend: trendRaw.map(d => ({ date: String(d.date).slice(5, 10), leads: Number(d.count), converted: 0 })),
        revenuePipeline: [],
      },
    };
  });
  ...
  fastify.get('/analytics/funnel', async (req) => {
    ...
    const funnel = await Promise.all(
      statuses.map(async s => ({
        name: s.name, value: await fastify.prisma.lead.count({ where: { tenantId, statusId: s.id } }),
      }))
    );
    return { success: true, data: funnel };
  });
