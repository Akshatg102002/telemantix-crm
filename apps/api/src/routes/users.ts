...
export async function userRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/users', { preHandler: [requireRole('admin', 'superadmin')] }, async (req, reply) => {
    ...
    const existing = await fastify.prisma.user.findFirst({ where: { email: body.email, tenantId: user.tenantId } });
    ...
    const createdUser = await fastify.prisma.user.create({
      data: { id: randomUUID(), tenantId: user.tenantId, name: body.name, email: body.email, passwordHash, role },
  ...
  fastify.patch('/users/:id', { preHandler: [requireRole('admin', 'superadmin')] }, async (req, reply) => {
    ...
    const existingUser = await fastify.prisma.user.findFirst({ where: { id, tenantId: user.tenantId } });
    if (!existingUser) return reply.code(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'User not found' } });
