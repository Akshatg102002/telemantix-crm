...
export async function automationRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/automations', { preHandler: [requireRole('admin', 'superadmin', 'manager')] }, async (req, reply) => {
    ...
    const automation = await fastify.prisma.automation.create({
      data: { id: randomUUID(), tenantId: user.tenantId, ...body, conditions: (body.conditions ?? []) as any, actions: body.actions as any, triggerConfig: body.triggerConfig as any },
    });
    reply.code(201);
    return { success: true, data: automation };
  });
  ...
  fastify.patch('/automations/:id', { preHandler: [requireRole('admin', 'superadmin', 'manager')] }, async (req, reply) => {
    ...
    const automation = await fastify.prisma.automation.findFirst({ where: { id, tenantId: user.tenantId, isSystem: false } });
    if (!automation) return reply.code(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'Not found or system rule' } });
    const updated = await fastify.prisma.automation.update({ where: { id }, data: { ...body, conditions: body.conditions as any, actions: body.actions as any, triggerConfig: body.triggerConfig as any } });
  ...
  fastify.get('/automations/:id/logs', async (req) => {
    ...
    const logs = await fastify.prisma.automationLog.findMany({
      where: { automationId: id, tenantId: user.tenantId },
      orderBy: { createdAt: 'desc' },
      take: 100,
    });
