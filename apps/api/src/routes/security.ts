...
export async function securityRoutes(fastify: FastifyInstance) {
  ...
  fastify.delete('/settings/sessions/:id', async (req) => {
    ...
    await fastify.prisma.userSession.deleteMany({ where: { id, userId: user.sub } });
  ...
  fastify.delete('/settings/sessions', async (req) => {
    ...
    await fastify.prisma.userSession.deleteMany({
      where: { userId: user.sub, refreshToken: { not: currentToken } },
    });
    return { success: true, data: null };
  });
  ...
  fastify.post('/auth/2fa/setup', async (req) => {
    ...
    const qrDataUrl = `otpauth://totp/Telemantix:${user.email}?secret=${secret}&issuer=Telemantix`;
  ...
  fastify.post('/auth/2fa/disable', async (req, reply) => {
    ...
    const currentUser = await fastify.prisma.user.findUnique({ where: { id: user.sub } });
    if (!currentUser) return reply.code(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'User not found' } });
  ...
  fastify.put('/settings/permissions', async (req) => {
    ...
    const tenant = await fastify.prisma.tenant.findUnique({ where: { id: user.tenantId } });
    const existingSettings = (tenant?.settings as Record<string, unknown>) || {};
    await fastify.prisma.tenant.update({
      where: { id: user.tenantId },
      data: { settings: { ...existingSettings, permissions: body } as any },
    });
  ...
  fastify.post('/settings/integration-secrets', async (req, reply) => {
    ...
    const secret = await fastify.prisma.integrationSecret.upsert({
      where: { tenantId_provider_keyName: { tenantId: user.tenantId, provider, keyName } },
  ...
  fastify.delete('/settings/integration-secrets/:id', async (req) => {
    const user = req.user as { sub: string; tenantId: string; role: string; email?: string };
    const { id } = req.params as { id: string };
    await fastify.prisma.integrationSecret.deleteMany({ where: { id, tenantId: user.tenantId } });
    return { success: true, data: null };
  });
