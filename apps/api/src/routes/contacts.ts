...
export async function contactRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/contacts/:id', async (req, reply) => {
    ...
    const contact = await fastify.prisma.contact.findFirst({ where: { id, tenantId: user.tenantId } });
  ...
  fastify.delete('/contacts/:id', async (req) => {
    const user = req.user as { sub: string; tenantId: string; role: string; email?: string };
    const { id } = req.params as { id: string };
    await fastify.prisma.contact.deleteMany({ where: { id, tenantId: user.tenantId } });
