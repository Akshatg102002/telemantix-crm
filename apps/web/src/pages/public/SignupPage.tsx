...
import { Check, ArrowRight, ArrowLeft, Eye, EyeOff, Zap } from 'lucide-react'
      ...
      {steps.map((label, i) => (
        <div key={label} className="flex items-center gap-2 flex-1">
          <div className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all duration-300 ${
            i < step ? 'bg-brand-gradient text-white' : i === step ? 'bg-brand-purple/20 border border-brand-purple text-brand-purple' : 'bg-bg-elevated border border-border text-text-muted'
          }`}>
...
export function SignupPage() {
  ...
  const handleSubmit = async (data: Step3Data) => {
    ...
    try {
      ...
      success('Welcome to Telemantix!', 'Your 14-day trial has started.')
      navigate('/dashboard')
    } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-bg flex">
      ...
      <div className="hidden lg:flex w-96 shrink-0 flex-col justify-between p-10 border-r border-border"
        ...
        <Link to="/" className="flex items-center gap-2.5">
          <div className="h-8 w-8 rounded-lg bg-brand-gradient flex items-center justify-center"><Zap className="h-4 w-4 text-white" /></div>
        ...
        <p className="text-text-muted text-xs">Already have an account? <Link to="/login" className="text-brand-purple hover:underline">Sign in</Link></p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-lg">
          <div className="mb-6 lg:hidden">
            <Link to="/" className="flex items-center gap-2"><div className="h-7 w-7 rounded-lg bg-brand-gradient flex items-center justify-center"><Zap className="h-3.5 w-3.5 text-white" /></div><span className="font-heading font-bold text-text-primary">Telemantix</span></Link>
          </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Select error={form1.formState.errors.industry?.message} {...form1.register('industry')}>
                        ...
                        {['Real Estate', 'Education', 'Finance', 'Healthcare', 'E-commerce', 'Technology', 'Other'].map(i => <option key={i} value={i}>{i}</option>)}
                      </Select>
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {plan.isPopular && <span className="text-[10px] bg-brand-gradient text-white px-2 py-0.5 rounded-full">Popular</span>}
                          </div>
                ...
                <form onSubmit={form3.handleSubmit(handleSubmit)} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Input type={showPw ? 'text' : 'password'} placeholder="Min 8 characters" error={form3.formState.errors.password?.message} {...form3.register('password')} />
                      <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted">{showPw ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Input type={showConfirmPw ? 'text' : 'password'} placeholder="Repeat password" error={form3.formState.errors.confirmPassword?.message} {...form3.register('confirmPassword')} />
                      <button type="button" onClick={() => setShowConfirmPw(!showConfirmPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted">{showConfirmPw ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
                    </div>
                  </div>
                  <label className="flex items-start gap-2.5 text-xs text-text-secondary cursor-pointer">
                    ...
                    <span>I agree to the <a href="#" className="text-brand-purple hover:underline">Terms of Service</a> and <a href="#" className="text-brand-purple hover:underline">Privacy Policy</a></span>
                  </label>
                  {form3.formState.errors.terms && <p className="text-xs text-danger">{form3.formState.errors.terms.message}</p>}
                  <div className="flex gap-3 pt-2">
                    <Button type="button" variant="secondary" onClick={() => setStep(1)}><ArrowLeft className="h-4 w-4" /> Back</Button>
